...
public class StoreWrapper {
    ...
    List<Integer> rowData = new ArrayList<>(Arrays.asList(
    ...
    CircularBuffer<Integer> buffer_;
    ...
    public StoreWrapper(int _seriesLength, int _seriesNumber, GraphMode _mode) {
        this._seriesLength = _seriesLength;
        this._seriesNumber = _seriesNumber;
        this._mode = _mode;
        this._drawSeriesLength = (int) (_seriesLength / ((double) PERIOD / FREQ)) + 1;
        buffer_ = new CircularBuffer<>(_seriesLength * _seriesNumber);
    }
    ...
    int drawingFrequency() {
        return (int) ((rowData.size()) / (double) _drawSeriesLength);
    }
    ...
    void updateBuffer(final int counter) {
        ...
        List<Integer> dataExtracted = utils_.extractRangeData(rowData, (counter - 1) * seriesSize, seriesSize);
...
//        }
    ...
    Point preparePoint(final List<Double> data) {
        int idx_ = buffer_.writeIndex() - 1;
        if (idx_ >= data.size()) {
            idx_ = data.size() - 1;
